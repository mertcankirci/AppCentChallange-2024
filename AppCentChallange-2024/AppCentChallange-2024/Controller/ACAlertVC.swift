...
class ACAlertVC: UIViewController {
    ...
    let containerView = ACContainerView()
    let titleLable = ACTitleLabel(textAlignment: .center, fontSize: 20)
    let messageLabel = ACBodyLabel(textAlignment: .center)
    let actionButton = ACButton(backgroundColor: .systemPink, title: "Ok")
